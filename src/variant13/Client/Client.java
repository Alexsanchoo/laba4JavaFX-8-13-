...
public class Client extends Application {
        ...
        showBtn.setOnAction(event -> {
            if (isConnected) {
                try {
                    ...
                    ObservableList<Shop> list = getShops(entry);
        ...
        addBtn.setOnAction(event -> {
            if(isConnected) {
                try {
                    ...
                    if(window.getShop() == null) {
                        out.writeUTF("error");
                        throw new RuntimeException("Close window");
                    }
                    String entry = getFormatToSend(window.getShop());
                    out.writeUTF(entry);
        ...
        editBtn.setOnAction(event -> {
            if(isConnected) {
                try {
                    ...
                    entry = getFormatToSend(window.getShop());
                    out.writeUTF(entry);
    ...
    private ObservableList<Shop> getShops(String entry) {
        String[] elements = entry.split("/");
        Shop[] shops = new Shop[elements.length / 6];

        int i = 0, j = 0;
        while (i < shops.length) {
            shops[i] = new Shop();
            shops[i].setId(Integer.parseInt(elements[j++]));
            shops[i].setCategory(elements[j++]);
            shops[i].setName(elements[j++]);
            shops[i].setAddress(elements[j++]);
            shops[i].setRetailSpace(Integer.parseInt(elements[j++]));
            shops[i++].setCapital(Double.parseDouble(elements[j++]));
        }

        ObservableList<Shop> items = FXCollections.observableArrayList(shops);
        return items;
    }


    private String getFormatToSend(Shop shop) {
        String result = "";
        result += shop.getId() + "/";
        result += shop.getCategory() + "/";
        result += shop.getName() + "/";
        result += shop.getAddress() + "/";
        result += shop.getRetailSpace() + "/";
        result += shop.getCapital() + "/";
