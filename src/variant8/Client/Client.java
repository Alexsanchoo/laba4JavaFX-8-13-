...
public class Client extends Application {
        ...
        showBtn.setOnAction(event -> {
            if (isConnected) {
                try {
                    ...
                    ObservableList<MilitaryTechnics> list = getTechnics(entry);
        ...
        addBtn.setOnAction(event -> {
            if(isConnected) {
                try {
                    ...
                    if(window.getTechnic() == null) {
                        out.writeUTF("error");
                        throw new RuntimeException("Close window");
                    }
                    String entry = getFormatToSend(window.getTechnic());
                    out.writeUTF(entry);
        ...
        editBtn.setOnAction(event -> {
            if(isConnected) {
                try {
                    ...
                    entry = getFormatToSend(window.getTechnic());
                    out.writeUTF(entry);
    ...
    private ObservableList<MilitaryTechnics> getTechnics(String entry) {
        String[] elements = entry.split("/");
        MilitaryTechnics[] technics = new MilitaryTechnics[elements.length / 6];

        int i = 0, j = 0;
        while (i < technics.length) {
            technics[i] = new MilitaryTechnics();
            technics[i].setId(Integer.parseInt(elements[j++]));
            technics[i].setCategory(elements[j++]);
            technics[i].setName(elements[j++]);
            technics[i].setCountry(elements[j++]);
            technics[i].setYear(Integer.parseInt(elements[j++]));
            technics[i++].setPrice(Double.parseDouble(elements[j++]));
        }

        ObservableList<MilitaryTechnics> items = FXCollections.observableArrayList(technics);
        return items;
    }


    private String getFormatToSend(MilitaryTechnics technic) {
        String result = "";
        result += technic.getId() + "/";
        result += technic.getCategory() + "/";
        result += technic.getName() + "/";
        result += technic.getCountry() + "/";
        result += technic.getYear() + "/";
        result += technic.getPrice() + "/";
